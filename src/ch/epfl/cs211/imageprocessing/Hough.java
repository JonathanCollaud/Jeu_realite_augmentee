...
public final class Hough implements Comparator<Integer> {
	...
	public void displayLines(PImage img) {
		computeLines(img);
		
		// TODO
	}
	
	private void computeLines(PImage img) {
		...
		// We take the accumulator without the margin
		for (int i = rDim + 1; i < rDim * phiDim - 1; i++) {
			image.pixels[i] = accumulator[i];
		}

		return image;*/	
	}
	
	public ArrayList<Integer> getBestCandidates(PImage img) {
